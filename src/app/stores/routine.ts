                ...
                for (let weekday = 0; weekday <= 6; weekday++) {
                    items.push(
                        periods.map((period: Period, pIndex: number) => {
                            ...
                            return section.schedules[key]
                                ? subjectMap[section.schedules[key]]
                                : null;
                        })
                    );
                }
            ...
            return (sectionId: string) => {
                ...
                return section.subjects
                    .map((subject: SectionSubject) => {
                        ...
                        item["remaining"] =
                            Number(item["count"] ?? 0) -
                            filledPeriodSubjects.filter((s) => s == item.uuid)
                                .length;
                        // console.log(item);
                        return item;
                    })
            ...
            sections.forEach((section: Section) => {
                ...
                for (let key of Object.keys(section.schedules)) {
                    ...
                    if (subjectId) {
                        ...
                        const info = ObjectUtils.extractPeriodInfo(
                            key,
                            section.periods
                        );
                        // console.log(
        ...
        addSection(section: Section) {
            if (
                this.sections.filter((item) => {
                    return (
                        item.name == section.name && item.grade == section.grade
                    );
                }).length
            ) {
        ...
        removeSubject(sectionId: string, subjectId: string) {
            ...
            if (section.subjects && section.subjects.length) {
                section.subjects.splice(
                    section.subjects.findIndex(
                        (item: SectionSubject) => item.uuid == subjectId
                    ),
                    1
                );
            }
